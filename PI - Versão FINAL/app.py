...
def empresas():
	...
	return render_template("empresas.html", empresas = Empresa.select())
...
@app.route ("/alterarempresa/")
def alterarempresa():
	if not session.has_key("usuario"):
	    return redirect("/login/")
	if session["usuario"]!= "admin":
	    return render_template("acessonegado.html")
	return render_template("alterarempresa.html",usuarioLogado=session['usuario'])

@app.route ("/pedidosdetalhes/<id>/")
def pedidosdetalhes(id):
	if not session.has_key("usuario"):
	    return redirect("/login/")
	if session["usuario"]!= "admin":
	    return render_template("acessonegado.html")
	pedidos = PedidoProduto.get_by_id(id)
	return render_template("pedidosdetalhes.html",usuarioLogado=session['usuario'], **locals())
	...
	if request.method=="POST":
		try:
			...
			if request.form['cnpj'] == "":
				raise Exception(u"Informe o cnpj da empresa")
			p = Empresa()
			p.nome = request.form['nome']
			p.cnpj = request.form['cnpj']
			p.foto_url = processa_upload("foto_url")
			p.save()
...
@app.route("/deletarempresa/<id>/")
def deletarempresa(id):
	Empresa.delete().where(Empresa.id == id).execute()
	Produto.delete().where(Empresa.id == id).execute()
...
@app.route("/alterarempresa/<id>/", methods=["GET","POST"])
def atualizarempresa(id):
    
	if request.method=="POST":
		try:
			if request.form['nome'] == "":
				raise Exception(u"nome nao informado")	
			if request.form['cnpj'] == "":
				raise Exception(u"cnpj nao informado")

			e = Empresa.get_by_id(id)
			e.nome = request.form['nome']
			e.cnpj = request.form['cnpj']
			e.foto_url = processa_upload("foto_url")
			e.save()

			return redirect("/empresas/")
		except Exception as error:
			return render_template("alterarempresa.html", id = id,  msg_error = error, form = request.form.to_dict())
	else:
		e = Empresa.get(id)
		return render_template("alterarempresa.html", id = id, form=model_to_dict(e))
